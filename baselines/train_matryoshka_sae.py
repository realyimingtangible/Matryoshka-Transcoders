...
class MatryoshkaSparseAutoencoder(nn.Module):
    def __init__(self, input_dim, matryoshka_config, jump_params=None):
        """
        Matryoshka Sparse Autoencoder (MSAE) with hierarchical nested structure.
        Encodes h_2 -> latent (full), applies nested top-k, and decodes (per-level)
        from cumulative latent slices to reconstruct h_2 (autoencoder).
        ...
        """
        super(MatryoshkaSparseAutoencoder, self).__init__()
        ...
        # Validate configuration (same checks as before)
        ...
        # Multiple decoders: one per level (cumulative latents -> reconstruct input_dim)
        self.decoders = nn.ModuleList([
            nn.Linear(level_dim, input_dim, bias=True)
            for level_dim in self.levels
        ])
        
        print(f"\n{'='*60}")
        print("Matryoshka Sparse Autoencoder Architecture:")
        print(f"{'='*60}")
        print(f"  Input dim (h_2): {self.input_dim}")
        print(f"  Latent dim (max): {self.latent_dim}")
    ...
    def forward(self, h_2, return_all_levels=False):
        ...
        Returns:
            If return_all_levels=True:
                reconstructions: List of h_2 reconstructions for each level
                z_sparse: Sparse latent representation
            Else:
                h_2_recon: Final reconstruction (from largest level)
                z_sparse: Sparse latent representation
        """
        ...
        for i, decoder in enumerate(self.decoders):
            ...
            # Reconstruct input activations (h_2)
            h_2_recon = decoder(z_level)
            reconstructions.append(h_2_recon)
...
# =================== Training (Matryoshka SAE) ===================

def train_matryoshka_sae(model, dataloader, num_epochs, learning_rate, 
                         model_save_path, lambda_sparse=0.01):
    """
    Train Matryoshka Sparse Autoencoder with multi-level reconstruction
    
    Args:
        model: MatryoshkaSparseAutoencoder instance
        dataloader: DataLoader with (h_2, h_1) pairs (h_1 unused here)
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(dataloader, desc=f"Epoch {epoch+1}/{num_epochs}") as pbar:
            for h_2, _ in pbar:  # h_1 present in dataset but SAE ignores it
                h_2 = h_2.to(device)
                
                # Forward pass - get all level reconstructions
                reconstructions, z_sparse = model(h_2, return_all_levels=True)
                
                # Compute multi-level reconstruction loss (target is h_2)
                recon_loss = 0
                for i, h_2_recon in enumerate(reconstructions):
                    loss_level = criterion(h_2_recon, h_2)
...
if __name__ == "__main__":
    ...
    h_1 = loaded["y"]  # Hidden layer activations (256-dim) - kept in dataset but unused by SAE
    h_0 = loaded["z"]  # Classification outputs (1-dim) - unused
    ...
    # Create dataset and dataloader (keep same structure: (h2, h1))
    ...
    for model_num in range(1):  # Change range for multiple runs
        print(f"\n{'='*60}")
        print(f"Training Matryoshka Sparse Autoencoder - Model {model_num}")
        print(f"{'='*60}\n")
        
        # Initialize model (input_dim = h_2 dim)
        ...
        model_save_path = f"./models/matryoshka_sae_{model_num}.pt"
        
        model = train_matryoshka_sae(
            model=model,
